...
- **Record:** Every Import Data line has left the **Draft**/**Error** state (each line
  is **Done** or **Ignored**) — the queue jobs enqueued when the import reached
  **Queue Done** (see `05-approve`) have all finished, one job per Import Data line.
...
equals **Finished**. The automation calls `action_recompute_queue_done_result()`,
which re-evaluates the job batch and, once no Import Data line is left
**Draft**/**Error**, moves the record to **Done**.

If any Import Data line is still in the **Error** state when the batch finishes, the
import stays in **Queue Done** — resolve the errored lines (retry or ignore them) so
the batch can be recomputed and the import can reach **Done**.
